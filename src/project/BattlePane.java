...
//import static project.ProgrammingSemester4.gameOverImg;
...
public class BattlePane extends StackPane{
    ...
    private boolean [] wasRunning= new boolean[4];// array for tracing end of motion #0 1running right #1 1 running left
    //#2 2 running right  #3 running left
    ...
    public BattlePane(ImageView bckground, Champion champion1, Champion champion2){
        ...
        this.computer = new ComputerPlayerMain(level.easy, c2);
                //this.p1 =new Player(0, "Main Player", c1, c2, numOfPlayers.one, level.easy);
        }
        else if (c2.getName().equals("Ultron")){
        this.computer = new ComputerPlayerMain(level.medium, c2);
                //this.p1 =new Player(0, "Main Player", c1, c2, numOfPlayers.one, level.medium);
        }
        else if (c2.getName().equals("Thanos")){
        this.computer = new ComputerPlayerMain(level.hard, c2);
                //this.p1 =new Player(0, "Main Player", c1, c2, numOfPlayers.one, level.hard);
        }
        
        
        for(int i=0;i<4;i++) wasRunning[i]=false;
        //Champion.setSingle(true);
        //add health bars here as a independent class
        
        comp = new Timeline(new KeyFrame(Duration.millis(c2.getSpeed()), e->{
            //comp.automatedPlay(p1, imagesPaneC1, healthBar1.getLifeRectangle(), this, imagesPaneC2);
        ...
        myKeys2= new KeyManager();
        
        this.setOnKeyPressed(e->{
           myKeys.setKeyState(true, e);
           myKeys2.setKeyState(true, e);
        });
        this.setOnKeyReleased(e->{
           myKeys.setKeyState(false, e);
           myKeys2.setKeyState(false, e);
        });
    ...
    public void startBattle(){
        ...
        bckground.setX(0);
        bckground.setY(0);
        bckground.setFitHeight(bounds.getHeight());
        bckground.setFitWidth(bounds.getWidth());
    ...
    public void updateMultiBattle(){
        ...
        if (myKeys.getKeyState(0)&&(!c1.getJumplock())&&(!c1.getGetHitlock())) ChampionAnimation.jump(c1);
        ...
        if (myKeys2.getKeyState(4)&&(!c2.getJumplock())&&(!c2.getGetHitlock())) ChampionAnimation.jump(c2);
        else if(myKeys2.getKeyState(5)&&(!c2.getGetHitlock())) {ChampionAnimation.moveRight(imagesPaneC2, c2);wasRunning[2]=true;}
        else if(myKeys2.getKeyState(6)&&(!c2.getGetHitlock())){ ChampionAnimation.moveLeft(imagesPaneC2, c2);wasRunning[3]=true;}
        else if(myKeys2.getKeyState(7)&&(!c2.getGetHitlock())&&(!c2.getHitlock())) ChampionAnimation.pauseToHit(imagesPaneC2, c2,c1,healthBar1.getLifeRectangle(),this,imagesPaneC1);
    ...
    public void updateSingleBattle(){
        ...
        comp.play();
        if( c1.checkIfDead()) gameOverAnim(c2, this);
        if( c2.checkIfDead()) gameOverAnim(c1, this);
